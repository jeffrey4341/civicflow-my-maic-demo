...
The default engine is a deterministic TypeScript pipeline. If `ANTHROPIC_API_KEY` is provided, selected language and classification steps can use optional model assistance while keeping the same structured workflow. In either mode, generated material remains a draft until a person reviews it.
