...
| --- | --- | --- |
| **Role-aware workflow** — actors with distinct permissions move work through a state machine | Citizen vs. officer vs. supervisor roles drive a conditional `CitizenCase` lifecycle: missing information and manual review take priority; high-risk cases then await a supervisor; approval returns the case to `routed`; reply release, explicit work start and human closure produce `in_progress → closed`. Citizen mobile route `/m`; officer console `/officer/*`. | `src/lib/ai/pipeline.ts`, `src/lib/types.ts` |
